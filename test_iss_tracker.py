...
def mock_redis():
    ...
    with patch('iss_tracker.redis_client') as mock_redis:
        ...
        # Return bytes instead of strings to match real Redis behavior
        mock_redis.lrange.return_value = [b'2024-079T12:00:00.000Z', b'2024-079T13:00:00.000Z']
        
        # Create a side_effect that returns bytes objects for hgetall
        def mock_hgetall_side_effect(key):
            data = {
                'iss_data:2024-079T12:00:00.000Z': {
                    b'epoch': b'2024-079T12:00:00.000Z',
                    b'x': b'1.0',
                    b'y': b'2.0',
                    b'z': b'3.0',
                    b'x_dot': b'3.0',
                ...
                'iss_data:2024-079T13:00:00.000Z': {
                    b'epoch': b'2024-079T13:00:00.000Z',
                    b'x': b'4.0',
                    b'y': b'5.0',
                    b'z': b'6.0',
                    b'x_dot': b'6.0',
...
def test_epochs(tracker_with_data, mock_redis):
    ...
    # Use patch to override the read_data method to prevent actual API calls
...
def test_closest_epoch(tracker_with_data, mock_redis):
    ...
    with patch.object(tracker_with_data, 'read_data'):
        with patch('datetime.datetime') as mock_datetime:
            ...
            # Override epochs method to avoid read_data call
            with patch.object(tracker_with_data, 'epochs') as mock_epochs:
                mock_epochs.return_value = ['2024-079T12:00:00.000Z', '2024-079T13:00:00.000Z']
                
                # Also need to patch get_state_vector_epoch
                with patch.object(tracker_with_data, 'get_state_vector_epoch') as mock_get_vector:
                    ...
                    mock_get_vector.return_value = mock_state_vector
...
def test_all_epochs_route(client, mock_redis):
    ...
    with patch('iss_tracker.ISSTracker.epochs') as mock_epochs:
        mock_epochs.return_value = ['2024-079T12:00:00.000Z', '2024-079T13:00:00.000Z']
        ...
        assert data['epochs'] == ['2024-079T12:00:00.000Z', '2024-079T13:00:00.000Z']
...
def test_all_epochs_route_with_limit_offset(client, mock_redis):
    ...
    with patch('iss_tracker.ISSTracker.epochs_limited') as mock_epochs:
        ...
        data = json.loads(response.data)
        assert 'epochs' in data
        assert data['epochs'] == ['2024-079T13:00:00.000Z']
